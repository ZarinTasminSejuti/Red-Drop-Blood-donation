...
const AdminAllDonationRequest = () => {
  ...
  const roleFilter = users?.filter((user) => userDetails.email === user?.email);

  const volunteerFound = roleFilter[0]?.role;
  ...
  return (
    <div>
      <div className="my-5 p-10 text-center">
        <h2 className="text-2xl">All Donation Requests</h2>
      </div>
      <div className="">
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg pb-3">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            {/* table body */}
            <tbody>
              {donationCollection.map((singleDonationData, index) => (
                <tr
                  key={index}
...
export default AdminAllDonationRequest;
